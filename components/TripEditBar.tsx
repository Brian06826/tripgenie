...
export function TripEditBar({ onSubmit, onUndo, canUndo, isLoading, error, language }: Props) {
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          {canUndo && !isLoading && (
            <button
              type="button"
              onClick={onUndo}
              className="shrink-0 px-3 py-2.5 text-xs font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors min-h-[44px]"
            >
              ↩ {isChinese ? '復原' : 'Undo'}
            </button>
          )}
